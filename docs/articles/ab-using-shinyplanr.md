...
The typical workflow for a *shinyplanr* analysis is: open the
**Scenario** tab → set targets for each feature → choose a cost layer →
optionally apply constraints → click **Run Analysis** → explore results
across the output tabs → use the **Comparison** tab to run two scenarios
side-by-side and examine trade-offs. Each of these steps is explained in
detail below.
...
application has been configured for your region, but typically include:
...
Depending on how the application is configured, targets can be set in
three ways:

**Individual Targets** Each feature has its own slider, allowing
fine-grained control. Adjust each slider to set the percentage of that
feature you want to protect.

**Category Targets** Features are grouped into categories (e.g.,
Habitat, Depth Zones, Spawning Areas). A single slider controls all
features within each category.

**Master Target** A single slider sets the same target for all features
at once. This is the simplest option for quick exploration.

> **Setting Appropriate Targets**
>
> For initial exploration, try setting targets around 30% (aligned with
...
- **Equal Area Cost**: Every planning unit has the same cost. The
  algorithm minimises total area selected.
- **Distance to Coast**: Planning units closer to shore generally have a
  higher cost, reflecting a proxy for higher production and fisheries
  value.
- **Fisheries Effort**: Areas with higher fishing activity have higher
  cost, minimising conflict with fisheries.
...
![](images/MinShortfall.png)

The budget is expressed as a percentage of the total cost layer. For
example:

- Budget = 30% means the total cost of selected planning units cannot
  exceed 30% of the total cost
...
>
> Some deployments use the **minimum set** objective instead, which
...
- **Climate Refugia**: Preferentially select areas projected to
  experience less climate impact
...
**Locked-In Areas** Areas that *must* be included in the solution:

- Existing marine protected areas
- Areas with conservation covenants
- Traditional protected areas

**Locked-Out Areas** Areas that *cannot* be included in the solution:
...
>
> Wait for the analysis to complete before clicking Run Analysis again.
...
![](images/TargetsTab.png)

For each feature, the chart shows:

- **Height of the bar**: The percentage actually achieved in the
  solution
- **Target**: Indicated by a bar with black outline and transparent
  white fill
...
![](images/ReportTab.png)

Click **Download Report** to create a self-contained HTML document
including:
...
![](images/FeatureDensity.png)

Planning units with higher feature density are more likely to be
selected because they contribute to multiple targets simultaneously.
...
- **Coverage Chart**: Bar chart showing how much of each feature falls
  within your polygons

![](images/UploadMap.png)

![](images/UploadCoverage.png)
...
The solver log shows “infeasible” when the problem has no valid
solution. Common causes:

- **Targets too high**: A feature target of 100% combined with
  locked-out areas that contain that feature makes the problem
  impossible. Reduce targets or remove conflicting constraints.
- **Budget too low** (minimum shortfall objective): The budget is so
  small that no planning units can be selected. Increase the budget
  percentage.
- **Conflicting constraints**: The same planning unit is both locked-in
  and locked-out. Check your constraint selections.
...
explains how to set up *shinyplanr* for a new region.
